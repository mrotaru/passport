...
module.exports = function authenticate(passport, name, options, callback) {
  ...
  return function authenticate(req, res, next) {
    ...
    function allFailed() {
      ...
      if (options.ajaxFailureMessage) {
          res.end(options.ajaxFailureMessage);
      } else {
          ...
          function complete() {
            if(options.ajax) {
                if(options.successRedirect) {
                    res.location(options.successRedirect);
                    res.status(278);
                    return res.end();
                } else {
                    res.writeHead(200, {"Content-Type": "text/plain"});
                }
                if(options.ajaxSuccessMessage) {
                    return res.end(options.ajaxSuccessMessage);
                } else {
                    return res.end();
                }
            }
